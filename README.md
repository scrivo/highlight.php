...
*highlight.php* is a server side code highlighter written in PHP that currently supports over 170 languages. It's a port of [highlight.js](http://www.highlightjs.org) by Ivan Sagalaev that makes full use of the language and style definitions of the original JavaScript project.
...
```bash
composer require scrivo/highlight.php:9.*
```
...
Add the autoload file generated by Composer to your `php` file:
...
This class has a very distinct purpose and might be useful in other projects as well (and may be a start for a new project ;) ).
...
Note that the following tests for _highlight.js_ are not rewritten for _highlight.php_:
...
If all went right you'll now have a file with all language definitions. Run `get_language_defintions.php` to generate the `json` files for the individual languages:
